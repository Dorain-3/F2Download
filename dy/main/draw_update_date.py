"""
日期分布统计工具（update_time）- 分析抖音用户数据按更新时间的分布情况

功能说明:
    本脚本读取right_urls.json文件，分析其中的日期分布数据，特别是update_time字段。
    支持区分is_update=1和is_update=0的数据，并生成柱状图和统计报告。
    与draw_old_date.py的区别在于：本脚本分析的是update_time（更新时间），而前者分析的是old_date（起始日期）。

主要功能:
    - analyze_date_distribution(): 分析日期分布数据（基于update_time）
...
def analyze_date_distribution(json_data):
    """
    分析日期分布并区分is_update的值（基于update_time字段）
    
    根据update_time字段统计不同is_update值的数据分布。
    ...
    for item in json_data["json_list"]:
        if item["is_update"] == "1":
            update_1_dates.append(item["update_time"])
        else:
            update_0_dates.append(item["update_time"])
